...
def process_pdf_by_client(uploaded_file, cliente_alvo: str):
    ...
    for line in extract_pdf_lines(uploaded_file):
        ...
        rebate_pdf = (
            parse_money(money_matches[2])
            if cliente_alvo in ["Oscar", "Demetra"] and len(money_matches) >= 3
            else 0.0
        )
...
def _extract_agent_from_full_text(img: Image.Image, aliases, display_name: str) -> dict:
    """Fallback robusto: lê a imagem inteira e procura a linha textual do agente.

    Espera o formato geral: AGENTE | RAKE | -25% | GANHOS | RESULTADO.
    Não depende da posição da linha nem de coordenadas fixas.
    """
    texts = []
    for psm in (6, 11, 4):
        txt = ocr_image(img, psm=psm)
        if txt.strip():
            texts.append(txt)
    ...
    for full in texts:
        for line in [ln.strip() for ln in full.splitlines() if ln.strip()]:
            # ignora cabeçalhos e rodapé
            upper = line.upper()
            if any(k in upper for k in ["SUPER AGENTE", "RESULTADO", "ADIANTAMENTO", "TOTAL"]):
                continue

            score = max((_name_similarity(line, t) for t in targets), default=0.0)
            # também compara token a token para nomes curtos/fragmentados
            for tok in re.split(r"\s+", line):
                score = max(score, max((_name_similarity(tok, t) for t in targets), default=0.0))
    ...
    if best_line is None or best_score < 0.45:
        return {
            ...
            "ganhos": 0.0,
            "rake": 0.0,
            "ocr_text": f"Fallback texto integral: agente não localizado. Melhor similaridade: {best_score:.2f}",
        }

    vals = extract_all_money_misto(best_line)
    # Linha esperada: rake, -25%, ganhos, resultado.
    # Se o OCR omitir alguma coluna, exige ao menos rake e ganhos identificáveis.
    rake = 0.0
    ganhos = 0.0
    if len(vals) >= 4:
        rake = vals[0]
        ...
        # Se houver três valores, o intermediário costuma ser -25%; usa o último como ganhos.
        rake = vals[0]
        ganhos = vals[2]
    elif len(vals) == 2:
        rake = vals[0]
    ...
    found = abs(rake) > 0.0001 or abs(ganhos) > 0.0001 or best_score >= 0.80
    return {
        "found": found,
        "agente": display_name,
        "ganhos": float(ganhos),
        "rake": float(rake),
        "ocr_text": (
            "MÉTODO: fallback por OCR da imagem inteira\n"
            f"Linha localizada: {best_line}\n"
            f"Similaridade: {best_score:.2f}\n"
            f"Valores monetários detectados: {vals}\n"
            f"RAKE FINAL: {rake}\n"
            f"GANHOS FINAL: {ganhos}"
        ),
    }
...
def extract_agent_from_adamantium_table(img: Image.Image, aliases, display_name: str) -> dict:
    ...
    # Segundo método: OCR da imagem inteira, procurando a linha textual do agente.
    full_text = _extract_agent_from_full_text(img, aliases, display_name)
    if full_text.get("found") and (abs(full_text.get("ganhos", 0.0)) > 0.0001 or abs(full_text.get("rake", 0.0)) > 0.0001):
        grid_reason = table.get("reason", "Agente não encontrado na grade.")
        full_text["ocr_text"] = (
            f"A leitura pela grade não foi concluída: {grid_reason}\n"
            "Foi usado o fallback por OCR da imagem inteira.\n\n"
            + full_text.get("ocr_text", "")
        )
        return full_text

    # Terceiro método: cabeçalhos e coordenadas detectadas pelo OCR.
    fallback = _extract_agent_by_headers(img, aliases, display_name)
    grid_reason = table.get("reason", "Agente não encontrado na grade.")
    fallback["ocr_text"] = (
        f"A leitura pela grade não foi concluída: {grid_reason}\n"
        "O fallback por OCR da imagem inteira também não encontrou valores confiáveis.\n"
...
def page_demetra():
    ...
    if pdf is not None:
        for _,r in process_pdf_by_client(pdf,"Demetra").iterrows():
            rb = rb_value(r["rake"], r["rb_percentual"])

            # No Demetra, o valor do REBATE do PDF é ignorado.
            # Ele apenas indica se o agente possui rebate.
            # Se houver rebate no PDF, recalcula 10% sobre GANHOS + RB.
            tem_rebate = bool(r.get("tem_rebate_pdf", False))
            rebate_calculado = -(float(r["ganhos"]) + rb) * 0.10 if tem_rebate else 0.0
    ...
    if imagem is not None:
        ...
        if not dados["found"] or (dados["ganhos"]==0 and dados["rake"]==0):
            ...
            rows.append(base_row("Killuminatti",dados["ganhos"],dados["rake"],70.0, rebate=0.0))
